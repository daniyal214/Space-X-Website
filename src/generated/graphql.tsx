...
export type LaunchesQuery = (
  ...
  & { launches?: Maybe<Array<Maybe<(
    { __typename?: 'Launch' }
    & Pick<Launch, 'details' | 'is_tentative' | 'launch_success' | 'launch_year' | 'mission_id' | 'mission_name' | 'upcoming'>
    & { launch_site?: Maybe<(
      { __typename?: 'LaunchSite' }
...
export type RocketsQueryVariables = Exact<{ [key: string]: any; }>;
...
export type RocketsQuery = (
  ...
  & { rockets?: Maybe<Array<Maybe<(
    { __typename?: 'Rocket' }
    & Pick<Rocket, 'rocket_id' | 'boosters' | 'active' | 'flickr_images' | 'company' | 'cost_per_launch' | 'country' | 'description' | 'first_flight' | 'rocket_name' | 'success_rate_pct'>
    & { diameter?: Maybe<(
      { __typename?: 'Dimension' }
      & Pick<Dimension, 'meters'>
    )>, engines?: Maybe<(
      { __typename?: 'Engines' }
      & Pick<Engines, 'number' | 'type' | 'version'>
    )>, mass?: Maybe<(
      { __typename?: 'Mass' }
      & Pick<Mass, 'kg'>
    )> }
...
export type ShipsQuery = (
  ...
  & { ships?: Maybe<Array<Maybe<(
    { __typename?: 'Ship' }
    & Pick<Ship, 'abs' | 'active' | 'attempted_landings' | 'class' | 'home_port' | 'ship_id' | 'image' | 'imo' | 'ship_model' | 'ship_name' | 'roles' | 'speed_kn' | 'status' | 'successful_landings' | 'ship_type' | 'weight_kg' | 'year_built'>
    & { missions?: Maybe<Array<Maybe<(
      { __typename?: 'BasicMission' }
      & Pick<BasicMission, 'flight' | 'name'>
    )>>> }
  )>>> }
);
